...
static bool check_boot_aggregate(pts_pcr_t *pcrs, chunk_t measurement,
								 char *algo, bool pcr_padding)
{
	chunk_t boot_aggregate, ba_measurement;
	...
	/* the boot aggregate hash is computed over PCR0 .. PCR7 */
	...
	if (pcr_ok)
	{
		...
		boot_aggregate = chunk_create(pcr_buffer,  hash_size);
	}
										 ...
										 pts_database_t *pts_db)
{
	...
	INIT(this,
		...
		.pcr_padding = lib->settings->get_bool(lib->settings,
						"%s.plugins.imc-attestation.pcr_padding", TRUE, lib->ns),
		.ref = 1,
	);
